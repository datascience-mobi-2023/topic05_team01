...
for (col in colnames(cor_prism_gastric)) {
  # Get the corresponding column name from df1
  df1_col <- df1[rownames(df2), col]

  # Find the matching column name in df3
  df3_col <- as.character(df3[df3$column_of_interest == df1_col, "column_name_to_match"])

  # Check if a matching column name was found in df3
  if (length(df3_col) > 0) {
    # Subset the columns from df2 and df3
    subset_df2 <- df2[, col]
    subset_df3 <- df3[, df3_col]
