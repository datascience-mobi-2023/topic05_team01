...
help("heatmap.2")
library(grid)
grid.text("Correlation heatmap", gp=gpar(fontsize=12, fontface="bold"))
heatmap.2(correlation_matrix,
          col = colors,
          xlab = "Drug",
          ylab = "Cell",
...
clusternum <- NbClust ((pc_scores_prism), distance="euclidean", method="kmeans")
+++
pam.res3 <- pam(pc_scores_prism, 3,  metric = "euclidean", stand = FALSE)
fviz_cluster(pam.res3, data = pc_scores_prism, palette = c("#FC4E07", "#00AFBB", "#E7B800"), ellipse.type = "euclid", 
star.plot = TRUE, 
repel = TRUE, 
ggtheme = theme_minimal(), geom = "point" )
...
library(cluster)
library(factoextra)
fviz_nbclust(pc_scores_prism_t, kmeans, method = "wss") + geom_vline(xintercept = 3, linetype=2)
 #it works ith pc_scpres_prism in the same way, so both are correct
...
```{r}
#trying something
rows_with_CDH1 <- subset(prism.snv, Hugo_Symbol == "CDH1")
print(rows_with_CDH1)
CDH1_DepMap_ID <- rows_with_CDH1$DepMap_ID


HDAC_inhibitor_df_all <- prism_celllines[, grepl(paste(HDAC_names_prism_new, collapse = "|"), colnames(prism_celllines))]


HDAC_names_new_selected_all <- colnames(HDAC_inhibitor_df_all)
...
View(moa_frequency_breast)
```

```{r}
# create a new dataframe of target frequency and select those which have more than 1 freq:

moa_freq_dataframe_selected_breast <- subset(moa_frequency_breast, Freq > 1)
...
ggplot(1:length(pca_variances_gastric_cleaned_treatments), pca_variances_gastric_cleaned_treatments, type = "b", xlab = "Principal Component", ylab = "Variance Explained",main = "Scree Plot")
plot(pca_values_gastric_cleaned_treatments[, 1], pca_values_gastric_cleaned_treatments[, 2], xlab = "PC1", ylab = "PC2")


scree_plot_g <- ggplot(data = data.frame(Principal_Component = 1:length(pca_variances_gastric_cleaned_treatments),
                                       Variance_Explained = pca_variances_gastric_cleaned_treatments),
...
pca_df_g <- data.frame(PC1 = pca_values_gastric_cleaned_treatments[, 1],
                     PC2 = pca_values_gastric_cleaned_treatments[, 2])

# Scatter plot using ggplot2
scatter_plot_gg <- ggplot(data = pca_df_g, aes(x = PC1, y = PC2)) +
  geom_point(color = "blue", size = 3) +
  labs(x = "PC1", y = "PC2") +
  ggtitle("Scatter Plot") +
  theme_minimal()
print(scatter_plot_gg)
# Combine the plots
combined_plot_g <- cowplot::plot_grid(scree_plot_g, scatter_plot_gg, nrow = 1)
...
```
```{r}
subset_snv_HDAC <- prism.snv_gastric[prism.snv_gastric$Hugo_Symbol == "HDAC1" ]
print(subset_snv_HDAC)
View(subset_snv_HDAC)
```
