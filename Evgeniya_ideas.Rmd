...
# Loop over each drug
for (j in 1:length(drug_names_prism)) {
  drug_matrix <- matrix(NA, nrow = nrow(prism_gastric_cleaned_treatments), ncol = sum(grepl(drug_names_prism[j], colnames(prism_gastric_cleaned_treatments))))
  for (i in 1:nrow(prism_gastric_cleaned_treatments)) {
    drug_response_try <- prism_gastric_cleaned_treatments[i, grepl(drug_names_prism[j], colnames(prism_gastric_cleaned_treatments))]
