...
## **3.1 Differential Treatment Efficacy in Oncological and Non-Oncological Drugs: A Fold Change Analysis**
...
plot1 <- ggplot(pca_data_prism.filtered1_drugs_variances, aes(x = PC1, y = PC2, color = Group)) +
  geom_point() +
  scale_color_manual(values = c("Bone cancer" = bone.cancer.celllines.group,"Kidney cancer" = kidney.cancer.celllines.group, "Liver cancer" = liver.cancer.celllines.group, "Gastric cancer" =  gastric.cancer.celllines.group, "Breast cancer" = breast.cancer.celllines.group)) +
  labs(x = "PC1", y = "PC2", title = "PCA Scatter Plot for drug response of five different cancer subtypes")

# Plot 2: PCA with K-means Clustering and Circles
plot2 <- ggplot(pc_scores_prism, aes(PC1, PC2, color = cluster)) +
  geom_point() +
...
## **3.6 Shared Targets and Mechanisms of Action in Gastric and Breast Cancer: Comparative Analysis**
...
# Arrange the plots side by side using cowplot
combined_plot <- plot_grid(bar_plot_breast, bar_plot_moa_breast, labels = c("A", "B"), ncol = 2)
...
plot1<-matplot(vector, vectors, type = "p", pch = 16, col = c("blue", "red", "green", "orange", "purple", "pink", "brown", "black"),
        xlab = "CNVs of HDAC1", ylab = "BRD-K56957086-001-06-3", cex = 0.4)
par(cex.lab = 0.6)  # Adjust the size of the axis titles with cex.lab argument

# Get the coordinates for the legend in the first plot
legend_x1 <- 1.2
legend_y1 <- 0.5

# Add a legend to the first plot
legend1 <- legend(legend_x1, legend_y1, legend = c("0.00061034", "0.0024414", "0.00976562", "0.0390625", "0.15625", "0.625", "2.5", "10"),
                  col = c("blue", "red", "green", "orange", "purple", "pink", "brown", "black"), pch = 16, xpd = TRUE, bty = "n")

# Create the second plot
plot2 <- matplot(vector, vectors, type = "p", pch = 16, col = c("blue", "red", "green", "purple", "pink", "brown", "black"),
                 xlab = "TPM values of HDAC1", ylab = "BRD-K56957086-001-06-3", cex = 0.4)
par(cex.lab = 0.6)
# Get the coordinates for the legend in the second plot
legend_x2 <- 9.11
legend_y2 <- 0.5
...
# Arrange the plots side by side using cowplot
combined_plot <- cowplot::plot_grid(plot1 + theme(legend.position = "none"), plot2 + theme(legend.position = "none"),
                                    nrow = 1)


# Add the legends to the combined plot
combined_plot_with_legends <- cowplot::plot_grid(legend1, combined_plot, legend2, ncol = 3, rel_widths = c(0.5, 1, 0.5))
combined_plot_with_legends <- combined_plot_with_legends +
...
# Create the first plot
plot1 <- matplot(vector, vectors, type = "p", pch = 16, col = c("blue", "red", "green", "orange", "purple", "pink", "brown", "black"),
                 xlab = "CNVs of BCR", ylab = "BRD-K35329391-334-01-7",cex = 0.4)
par(cex.lab=0.6)


# Create the second plot
plot2 <- matplot(vector, vectors, type = "p", pch = 16, col = c("blue", "red", "green", "orange", "purple", "pink", "brown", "black"),
                 xlab = "TPM values of BCR", ylab = "BRD-K35329391-334-01-7", cex=0.4)
par(cex.lab=0.6)
# Get the coordinates for the legend
...
# Add the legends to the combined plot
combined_plot <- cowplot::plot_grid(combined_plot, legend1, legend2, rel_widths = c(1, 0.1), align = "hv", axis = "l")
combined_plot
