...
# Filter for non-oncology treatments
non_oncology_treatments <- prism.treat[!grepl("oncology", prism.treat$disease.area, ignore.case = TRUE), ]

# Extract treatment IDs for non-oncology treatments
non_oncology_treatment_ids <- rownames(non_oncology_treatments)

# Filter prism data for these treatment IDs
non_oncology_responses <- prism_gastric[, colnames(prism) %in% non_oncology_treatment_ids]

# Impute NA values with column means
non_oncology_responses <- as.data.frame(lapply(non_oncology_responses, function(x) ifelse(is.na(x), mean(x, na.rm = TRUE), x)))

avg_response_non_oncology <- rowMeans(non_oncology_responses)

# Calculate the fold change for each cell line
pseudo_count <- 1e-9
fold_changes_oncological_2 <- (avg_response_non_oncology + pseudo_count) / (avg_response_oncology_gastric + pseudo_count)

# Check for errors: This should be fold_changes_oncological_2
if (length(names(avg_response_oncology_gastric)) == length(fold_changes_oncological_2)) {
    cell_lines <- names(avg_response_oncology_gastric)
} else {
    # Provide a default value, or handle this case as needed
    cell_lines <- 1:length(fold_changes_oncological_2) 
}

# Create the data frame using cell_lines for the cell_line column
fold_changes_gastric_df_2 <- data.frame(cell_line = cell_lines, fold_change = fold_changes_oncological_2)  

# Sort the dataframe by fold_change 
fold_changes_gastric_df_2 <- fold_changes_gastric_df_2[order(fold_changes_gastric_df_2$fold_change, decreasing = TRUE), ]
...
ggplot(fold_changes_gastric_df_2, aes(x = reorder(cell_line, -fold_change), y = fold_change, fill = cell_line)) +
    geom_bar(stat = "identity") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    labs(title = "Fold Changes Non-Oncology Treatments Relative to Oncology Gastric Treatments",
         x = "Cell Lines",
         y = "Fold Change",
         fill = "Cell Line") +
    guides(fill = FALSE)  # This removes the legend
...
ggplot(fold_changes_df, aes(x = reorder(cell_line, -fold_change), y = fold_change, fill = cell_line)) +
    geom_bar(stat = "identity") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
