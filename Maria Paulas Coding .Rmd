...
pseudo_count <- 1
# Calculate the fold change for each cell line with pseudo count
fold_changes_oncological <- (avg_response_non_oncology_gastric + pseudo_count) / (avg_response_oncology_gastric + pseudo_count)
...
#For avg_response_oncology_gastric, the p-value is 0.2409, which is greater than 0.05. This indicates that the data does not significantly deviate from a normal distribution.

# For avg_response_non_oncology_gastric, the p-value is 0.002525, which is less than 0.05. This indicates that the data is not normally distributed.
...
# The result of the Mann-Whitney U test shows that the p-value is extremely small (p-value \< 2.2e-16), which is much less than the common significance level of 0.05.

# This indicates that there is a statistically significant difference between the two groups (avg_response_non_oncology_gastric and avg_response_oncology_gastric). -\> there is very strong evidence to reject the null hypothesis (which states that there is no difference between the groups).

# Conclusion: statistically significant difference in responses between the non-oncology treatments and oncology treatments related to gastric or gastrointestinal cancer.
...
cell_line_names <- rownames(gastric_oncology_responses)
fold_changes_gastric_df <- data.frame(cell_line = cell_line_names, fold_change = fold_changes_oncological)
...
# Using ggplot2
p <- ggplot(fold_changes_gastric_df, aes(x = cell_line, y = fold_change)) +
  geom_bar(stat = "identity", fill = "pink") +
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size = 10)) + 
  labs(title = "Fold Changes of Non-Oncology Treatments Relative to Oncology Treatments", x = "Cell Lines", y = "Fold Change")
...
```{r}
p <- ggplot(fold_changes_gastric_df, aes(y = cell_line, x = fold_change)) +
  geom_bar(stat = "identity", fill = "pink") + 
  theme_minimal() + 
  theme(axis.text.y = element_text(size = 8)) + 
  labs(title = "Fold Changes of Non-Oncology Treatments Relative to Oncology Treatments", y = "Cell Lines", x = "Fold Change")
...
pseudo_count <- 1
# Calculate the fold change for each cell line with pseudo count
fold_changes <- (avg_response_non_oncology + pseudo_count) / (avg_response_oncology + pseudo_count)
